...
void do_bgfg(char **argv) 
{
  ...
  if(!(strcmp(argv[0],"fg")))  {
    job->state = FG; //changes the state to bg
    kill(-job->pid, SIGCONT); //tells the system to run the job again
    waitfg(job->pid); //we need this waitfg because we need to have only one fg process running at a time
  }
  else if(!(strcmp(argv[0],"bg")))  {
    job->state = BG; //changes the state to bg
    kill(-job->pid, SIGCONT); //tells the system to run the job again
...
void sigint_handler(int sig) 
{
    ...
    if (fgpid(jobs) > 0){ //must be greater than zero to be a fg job, 0 means no fg process found
        kill(-(fgpid(jobs)), sig); //kill the process - this is the default action for sigint
    }
    return;
}
